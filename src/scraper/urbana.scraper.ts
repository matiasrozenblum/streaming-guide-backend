  ...
  try {
    ...
    const results: { programs: any[]; logs: string[] } = await page.evaluate(
      () => {
        ...
        const dayHeaders = Array.from(table.querySelectorAll('thead th')).slice(
          1,
        );
        const days = dayHeaders.map(
          (th) => th.textContent?.trim().toLowerCase() || 'desconocido',
        );
        ...
        rows.forEach((row) => {
          ...
          cells.forEach((cell) => {
            const name = cell
              .querySelector('.event_header')
              ?.textContent?.trim();
            const time = cell
              .querySelector('.hours_container')
              ?.textContent?.trim();
            const panelistsRaw = cell
              .querySelector('.before_hour_text')
              ?.textContent?.trim();
            ...
            if (name && time) {
              ...
              const panelists = panelistsRaw
                ? panelistsRaw.split(',').map((p) => p.trim())
                : [];
