...
export async function scrapeGelatinaSchedule(): Promise<GelatinaProgram[]> {
  ...
  // Esperar a que los elementos de los programas estén presentes
  ...
  const data = await page.evaluate(() => {
    ...
    const formatTime = (input) => {
        const clean = input.replace(/[^\d:]/g, ''); // elimina letras como "h"
        if (!clean.includes(':')) {
          return `${clean}:00`;
        }
        return clean;
    ...
    programRows.forEach((row, index) => {
      if (index !== 1 && index % 2 === 0 || index > 13) return; // Saltar separadores
