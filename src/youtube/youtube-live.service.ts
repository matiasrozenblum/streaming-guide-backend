...
export class YoutubeLiveService {
  ...
  async getBatchLiveStreams(
    ...
    // First, pre-fetch all keys to avoid N+1 Redis queries
    const channelHandles = new Map<string, string>(); // Map channelId to handle for logging
    const handles = channelIds.map((id) => {
      const handle = channelHandleMap?.get(id) || 'unknown';
      channelHandles.set(id, handle);
      return handle;
    });

    const notFoundKeys = handles.map((h) => `videoIdNotFound:${h}`);
    const attemptKeys = handles.map((h) => `notFoundAttempts:${h}`);
    const statusKeys = handles.map((h) => `liveStatusByHandle:${h}`);

    // Fetch all keys in parallel
    const [notFoundResults, attemptResults, statusResults] = await Promise.all([
      this.redisService.mget<string>(notFoundKeys),
      this.redisService.mget<AttemptTracking>(attemptKeys),
      this.redisService.mget<LiveStatusCache>(statusKeys),
    ]);
    ...
    for (let i = 0; i < channelIds.length; i++) {
      ...
      // Check unified cache
      const cachedStatus = statusResults[i];
      if (cachedStatus) {
        try {
  ...
  private async getLiveStreamsInternal(
    ...
    try {
      ...
      const visibleCurrentSchedule = currentScheduleEntries.find(
        (s) => s.program?.is_visible !== false,
      );

      if (currentScheduleEntries.length > 0 && !visibleCurrentSchedule) {
        this.logger.debug(
          `🚫 Skipping ${handle} (${channelId}) - current program is not visible`,
        );
        return '__SKIPPED__';
      }
