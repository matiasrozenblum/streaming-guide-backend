...
export class YoutubeLiveService {
  ...
  async fetchLatestVideoUrl(feedUrl: string): Promise<string> {
    try {
      // Traemos como texto el XML
      const response = await axios.get<string>(feedUrl, {
        responseType: 'text',
      });

      // Parseamos el XML a objeto JS
      const parsed = await parseStringPromise(response.data);
      const entries = parsed.feed?.entry;
      if (!entries?.length) {
        throw new HttpException(
          'No se encontraron videos en la playlist',
          HttpStatus.NOT_FOUND,
        );
      }

      // Tomamos la primera entrada
      const firstEntry = entries[0];

      // Buscamos el <link rel="alternate" href="...">
      const linkObj = (firstEntry.link || []).find(
        (l: any) => l.$?.rel === 'alternate',
      );
      const url = linkObj?.$.href;
      if (!url) {
        throw new HttpException(
          'No se pudo extraer el enlace del último video',
      ...
      if (axios.isAxiosError(err) && err.response) {
        // Reenvía el status y mensaje de YouTube en caso de 4xx/5xx
        throw new HttpException(
      ...
      throw new HttpException(
        'Error interno al procesar el feed de YouTube',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
