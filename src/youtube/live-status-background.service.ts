...
export class LiveStatusBackgroundService {
  ...
  async getLiveStatusForChannels(
    ...
    const keys = handles.map((handle) => `${this.CACHE_PREFIX}${handle}`);
    const cachedStatuses = await this.redisService.mget<LiveStatusCache>(keys);

    // Parallelize shouldUpdateCache checks to avoid sequential waiting
    const updateChecks = await Promise.all(
      cachedStatuses.map(async (cached) => {
        if (cached) {
          return await this.shouldUpdateCache(cached);
        }
        return false;
      }),
    );
    ...
    handles.forEach((handle, index) => {
      // Handle the case where handle is falsy
      if (!handle) {
        return;
      }
      ...
      if (cached) {
        this.logger.debug(`[LIVE-STATUS-BG] Cache hit for ${handle}`);
        const needsUpdate = updateChecks[index];
        if (!needsUpdate) {
