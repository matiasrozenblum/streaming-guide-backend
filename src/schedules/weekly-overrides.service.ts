    ...
    if (keys.length > 0) {
      ...
      if (expiredKeys.length > 0) {
        ...
        for (let i = 0; i < expiredKeys.length; i += CHUNK_SIZE) {
          await this.redisService.del(expiredKeys.slice(i, i + CHUNK_SIZE));
        }
    ...
    if (keys.length > 0) {
      ...
      if (keysToDelete.length > 0) {
        ...
        for (let i = 0; i < keysToDelete.length; i += CHUNK_SIZE) {
          await this.redisService.del(keysToDelete.slice(i, i + CHUNK_SIZE));
        }
