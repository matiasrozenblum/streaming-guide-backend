...
export class WeeklyOverridesService {
  ...
  async cleanupExpiredOverrides(): Promise<number> {
    ...
    if (keys.length > 0) {
      ...
      // Delete expired overrides in a single batch to avoid N+1 Redis round-trips
  ...
  async deleteOverridesForProgram(
    ...
    // OPTIMIZATION: Use Redis pipeline to fetch and process matching keys, then batch delete to eliminate O(N) roundtrips
    const pipeline = (this.redisService as any).client.pipeline();
    keys.forEach((key) => pipeline.get(key));
    const results = await pipeline.exec();
