...
export class WeeklyOverridesService {
  ...
  async deleteOverridesForProgram(
    ...
    for (let i = 0; i < keys.length; i += batchSize) {
      ...
      // ⚡ Bolt Optimization: Replace individual gets with batch mget to prevent N+1 queries
      ...
      overrides.forEach((override, index) => {
        ...
        if (
          ...
          keysToDelete.push(chunk[index]);
        }
      });
    }

    if (keysToDelete.length > 0) {
      // ⚡ Bolt Optimization: Batch delete instead of deleting one by one
