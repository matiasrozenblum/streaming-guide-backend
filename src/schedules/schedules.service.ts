...
export class SchedulesService {
  ...
  async enrichSchedules(
    ...
    const batchStreamsResults = new Map<string, any>();
    ...
    for (const schedule of schedulesWithoutChannels) {
      ...
      let isLive = false;
      const streamUrl = program.stream_url || program.youtube_url;
  ...
  async create(dto: CreateScheduleDto): Promise<Schedule> {
    ...
    const schedule = this.schedulesRepository.create({
      ...
      schedule_type: (dto.scheduleType as any) ?? 'weekly',
      week_number_in_month: dto.weekNumberInMonth ?? null,
  ...
  async update(id: string, dto: UpdateScheduleDto): Promise<Schedule> {
    ...
    if (dto.scheduleType !== undefined)
      schedule.schedule_type = dto.scheduleType as any;
    if (dto.weekNumberInMonth !== undefined)
      schedule.week_number_in_month = dto.weekNumberInMonth;
  ...
  private getDayDateInCurrentWeek(dayOfWeek: string): dayjs.Dayjs {
    ...
    const monday = today.add(daysToMonday, 'day').startOf('day');
    const offsetFromMonday = targetDow === 0 ? 6 : targetDow - 1;
    return monday.add(offsetFromMonday, 'day');
  }
  ...
  private isNthWeekdayOfMonth(
    ...
    const targetDow = DAYS.indexOf(dayOfWeek);
    if (date.day() !== targetDow) return false;
    if (n === -1) {
    ...
    while (!d.isAfter(date)) {
      if (d.day() === targetDow) count++;
      d = d.add(1, 'day');
    }
    return count === n;
  }

  private scheduleOccursOnDate(schedule: any, date: dayjs.Dayjs): boolean {
    const DAYS = [
      'sunday',
      'monday',
      'tuesday',
    ...
    if (type === 'weekly') return schedule.day_of_week === DAYS[date.day()];
    if (type === 'monthly_weekday') {
      if (!schedule.day_of_week || schedule.week_number_in_month == null)
        return false;
      return this.isNthWeekdayOfMonth(
        date,
        schedule.day_of_week,
        schedule.week_number_in_month,
      );
    }
    if (type === 'monthly_dated') {
      if (!schedule.specific_date) return false;
      return schedule.specific_date === date.format('YYYY-MM-DD');
    }
    return false;
  }

  /**
   * Filters schedules for a given day or the current week.
   * - weekly: kept only if day_of_week matches.
   * - monthly_weekday: kept only if the schedule's Nth-weekday falls on the target date.
   * - monthly_dated: kept only if specific_date matches; day_of_week is computed and set.
   */
  private filterSchedulesByContext(
    schedules: any[],
    dayOfWeek?: string,
  ): any[] {
    const DAYS = [
    ...
    if (dayOfWeek) {
      const targetDate = this.getDayDateInCurrentWeek(dayOfWeek);
      return schedules
        .filter((s) => {
          const type = s.schedule_type || 'weekly';
          if (type === 'weekly') return s.day_of_week === dayOfWeek;
          return this.scheduleOccursOnDate(s, targetDate);
        })
        .map((s) => {
          if (s.schedule_type === 'monthly_dated' && s.specific_date) {
            const d = TimezoneUtil.parseInArgentinaTime(s.specific_date);
            return { ...s, day_of_week: DAYS[d.day()] };
          }
          return s;
        });
    }

    // Week view: always include weekly; monthly only if they fall in the current week
    const today = TimezoneUtil.now();
    const todayDow = today.day();
    const daysToMonday = todayDow === 0 ? -6 : -(todayDow - 1);
    const monday = today.add(daysToMonday, 'day').startOf('day');
    ...
    return schedules
      .filter((s) => {
        ...
        while (!d.isAfter(sunday)) {
          if (this.scheduleOccursOnDate(s, d)) return true;
          d = d.add(1, 'day');
        }
        return false;
      })
