...
export class WeeklyOverridesController {
  ...
  async deleteOverride(@Param('overrideId') overrideId: string) {
    const deleted =
      await this.weeklyOverridesService.deleteWeeklyOverride(overrideId);
    return {
      success: deleted,
  ...
  async getWeekOverrides(@Param('targetWeek') targetWeek: 'current' | 'next') {
    const weekStartDate =
      this.weeklyOverridesService.getWeekStartDate(targetWeek);
  ...
  async cleanupExpired() {
    ...
    return {
      success: true,
      message: `Cleaned up ${cleaned} expired overrides`,
    };
  }
}
