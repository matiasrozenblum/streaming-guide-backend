...
export class StreamerSubscriptionService {
  ...
  async subscribe(
    user: User,
    streamerId: number,
    dto: CreateStreamerSubscriptionDto,
  ): Promise<UserStreamerSubscription> {
  ...
  async notifySubscribers(
    ...
    this.logger.log(
      `📢 Notifying subscribers for streamer ${streamer.name} (ID: ${streamerId})`,
    );
    ...
    if (platform && streamer.services) {
      const activeService = streamer.services.find(
        (s) => s.service.toLowerCase() === platform.toLowerCase(),
      );
    ...
    this.logger.log(
      `✅ Notifications sent to ${subscriptions.length} subscribers.`,
    );
  }
}
