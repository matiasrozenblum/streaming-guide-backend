...
export class StreamerLiveStatusService {
  ...
  async syncLiveStatusFromTwitch(
    ...
    try {
      this.logger.log(
        `🔄 Syncing live status from Twitch API for streamer ${streamerId} (${username})`,
      );
      ...
      if (!clientId || !accessToken) {
        this.logger.warn(
          '⚠️ Twitch credentials not configured, cannot sync live status',
        );
        return {
          success: false,
          isLive: false,
      ...
      if (!response.ok) {
        const errorText = await response.text();
        this.logger.warn(
          `⚠️ Failed to fetch Twitch channel data for ${username}: ${response.status} - ${errorText}`,
        );
        return {
          success: false,
          isLive: false,
      ...
      this.logger.log(
        `📡 Twitch API response for ${username}: isLive=${isLive}`,
      );
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(
        `❌ Error syncing live status from Twitch for ${username}: ${errorMessage}`,
      );
      return { success: false, isLive: false, error: errorMessage };
    }
  }
