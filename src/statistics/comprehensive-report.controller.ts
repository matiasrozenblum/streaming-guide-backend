...
export class ComprehensiveReportController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Channel report generated successfully',
  })
  async generateChannelReport(
    ...
    try {
      const result =
        await this.comprehensiveReportService.generateChannelReport(
          channelId,
          body.from,
          body.to,
          body.format,
      ...
      if (
        body.action === ReportAction.DOWNLOAD &&
        result &&
  ...
  @Post('periodic')
  @ApiOperation({
    summary: 'Generate periodic report (weekly, monthly, quarterly, yearly)',
  })
  @ApiBody({ type: PeriodicReportDto })
  @ApiResponse({
    status: 200,
    description: 'Periodic report generated successfully',
  })
  async generatePeriodicReport(
    ...
    try {
      const result =
        await this.comprehensiveReportService.generatePeriodicReport(
          body.period,
          body.from,
          body.to,
          body.channelId,
  ...
  async generateManualWeeklyReport(@Query('channelId') channelId?: number) {
  ...
  async generateManualMonthlyReport(@Query('channelId') channelId?: number) {
    return this.comprehensiveReportService.generatePeriodicReport(
  ...
  async generateManualQuarterlyReport(@Query('channelId') channelId?: number) {
    return this.comprehensiveReportService.generatePeriodicReport(
  ...
  async generateManualYearlyReport(@Query('channelId') channelId?: number) {
    return this.comprehensiveReportService.generatePeriodicReport(
      ReportPeriod.YEARLY,
      undefined,
      undefined,
