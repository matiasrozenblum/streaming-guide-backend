export class SimilarityUtil {
  ...
  static calculateTitleSimilarity(
    ...
    if (
      videoLower.includes(programLower) ||
      programLower.includes(videoLower)
    ) {
    ...
    const meaningfulWordsMatched = programWords.filter((word) =>
      videoLower.includes(word),
    ).length;
    const meaningfulWordsRatio =
      programWords.length > 0
        ? meaningfulWordsMatched / programWords.length
        : 0;
    ...
    const intersection = new Set(
      [...allWords1].filter((x) => allWords2.has(x)),
    );
