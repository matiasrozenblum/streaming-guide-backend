...
export class AddPerformanceIndexes1734567890000 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_schedule_day_time_visible"`,
    );
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_user_subscription_active"`,
    );
    await queryRunner.query(`DROP INDEX IF EXISTS "idx_device_device_id"`);
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_program_panelist_program_id"`,
    );
  }
}
