...
export class AddUserOrigin1747430368001 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."users_origin_enum" AS ENUM('traditional', 'google', 'facebook')`,
    );

    // Add the origin column with default value 'traditional' for existing users
    await queryRunner.query(
      `ALTER TABLE "users" ADD "origin" "public"."users_origin_enum" NOT NULL DEFAULT 'traditional'`,
    );

    // Add an index for better query performance
    await queryRunner.query(
      `CREATE INDEX "IDX_users_origin" ON "users" ("origin")`,
    );
  }
