  ...
  if (successfulResults.length > 0) {
    ...
    if (cacheTimes.length > 1) {
      const cacheImprovement =
        ((cacheTimes[0] - cacheTimes[cacheTimes.length - 1]) / cacheTimes[0]) *
        100;
      console.log(`\n💾 Cache Performance:`);
    ...
    if (baselineResult.success && bulkFetchResult.success) {
      const improvement =
        ((baselineResult.time - bulkFetchResult.time) / baselineResult.time) *
        100;
      ...
      console.log(
        `   Performance impact: ${improvement > 0 ? '+' : ''}${improvement.toFixed(2)}%`,
      );
    }
  }
